...
\end{equation}
For $F$ and $B$ we use each side of the constraint from (\ref{eq:OCP_FE}). If the constraint holds for all $\varphi_i$ it holds for all $v_h\in V_H$ which gives us the forms,
...
To solve to equation (\ref{eq:OCP_coeff}) we use Lagrange multipliers and the Lagrangian
\begin{align}
